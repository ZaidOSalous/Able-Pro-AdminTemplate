<script>
  ...
  let clipboard;
  onMount(() => {
    clipboard = new ClipboardJS('.color-block');

    clipboard.on('success', (e) => {
      showBadge(e.trigger, 'bg-success', 'copied');
    });

    clipboard.on('error', (e) => {
      showBadge(e.trigger, 'bg-danger', 'Error');
    });
  });

  function showBadge(element, badgeClass, message) {
    const iconBadge = document.createElement('span');
    iconBadge.className = `ic-badge badge ${badgeClass} float-end`;
