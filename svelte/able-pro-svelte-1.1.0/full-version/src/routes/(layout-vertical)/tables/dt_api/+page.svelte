<script>
  ...
  onMount(() => {
    // [ Add Rows ]
    var t = new DataTable('#add-row-table');
    var counter = 1;
    ...
    new DataTable('#footer-select', {
      initComplete: function () {
        this.api()
          .columns()
          .every(function () {
            var column = this;
            var select = jQuery('<select class="form-control form-control-sm"><option value=""></option></select>')
    ...
    var srow = new DataTable('#row-select');

    jQuery('#row-select tbody').on('click', 'tr', function () {
      jQuery(this).toggleClass('selected');
    });

    var drow = new DataTable('#row-delete');

    jQuery('#row-delete tbody').on('click', 'tr', function () {
      if (jQuery(this).hasClass('selected')) {
        jQuery(this).removeClass('selected');
      } else {
        drow.jQuery('tr.selected').removeClass('selected');
        jQuery(this).addClass('selected');
      }
    });

    jQuery('#row-delete-btn').on('click', function () {
      drow.row('.selected').remove().draw(!1);
    });

    // [ Form input ]
    var table = new DataTable('#form-input-table');

    jQuery('#form-input-btn').on('click', function () {
      var data = table.jQuery('input, select').serialize();
      alert('The following data would have been submitted to the server: \n\n' + data.substr(0, 120) + '...');
      return false;
    });

    // [ Show-hide table js ]
    var sh = new DataTable('#show-hide-table', {
      scrollY: '200px',
      paging: false
    });
    ...
    jQuery('a.toggle-vis').on('click', function (e) {
      ...
      var column = sh.column(jQuery(this).attr('data-column'));
    ...
    function filterGlobal() {
      jQuery('#search-api')
        .DataTable()
        .search(jQuery('#global_filter').val(), jQuery('#global_regex').prop('checked'), jQuery('#global_smart').prop('checked'))
        .draw();
    }
...
<div class="pc-container">
  <div class="pc-content">
    <!-- [ breadcrumb ] start -->
    <Breadcrumb item="DataTable" itemActive="API" />
    <!-- [ breadcrumb ] end -->
    <!-- [ Main Content ] start -->
    <div class="row">
      <!-- prettier-ignore -->
      ...
      <div class="col-sm-12">
        <div class="card">
          <div class="card-header">
            <h5>Add Rows</h5>
            <small
              >New rows can be added to a DataTable using the row.add() API method. Simply call the API function with the data for the new
              row
            </small>
          </div>
          <div class="card-body">
      ...
      <div class="col-sm-12">
        <div class="card">
          <div class="card-header">
            ...
            <small
              >It can be useful to provide the user with the option to select rows in a DataTable. This can be done by using a click event
              to add / remove a class on the table rows.</small
            >
          </div>
          <div class="card-body">
      ...
      <div class="col-sm-12">
        <div class="card">
          <div class="card-header">
            <h5>Form Inputs</h5>
            <small
              >In order to perform paging, ordering, searching etc, DataTables can remove rows and cells from the document (i.e. those rows
              / cells which are not needed are not inserted into the document).</small
            >
          </div>
          <div class="card-body">
        ...
        <div class="card">
          <div class="card-header">
            ...
            <small
              ...
              Also included here is scrolling, just to show it enabled with this API method, although it's not required for the API function
              to work.</small
            >
          </div>
          <div class="card-body">
      ...
      <div class="col-sm-12">
        <div class="card">
          <div class="card-header">
            ...
            <small
              >Searching a table is one of the most common user interactions with a DataTables table, and DataTables provides a number of
              methods for you to control this interaction.</small
            >
          </div>
          <div class="card-body">
            <div class="dt-responsive table-responsive">
