...
<script src="<?= base_url('assets/js/plugins/clipboard.min.js') ?>"></script>
    <script>
      // Wait for the window to load before initializing ClipboardJS
      window.addEventListener('load', () => {
        // Initialize ClipboardJS on elements with the 'color-block' class
        const clipboard = new ClipboardJS('.color-block');

        // Handle the 'success' event when copying succeeds
        clipboard.on('success', (e) => {
          const targetElement = e.trigger;

          // Check if the target element already contains a badge to prevent duplicates
          if (!targetElement.querySelector('.ic-badge')) {
            const successBadge = createBadge('copied', 'bg-success');

            // Append the success badge and remove it after 3 seconds
            targetElement.append(successBadge);
            removeBadgeAfterDelay(successBadge, 3000);
          }
        });

        // Handle the 'error' event when copying fails
        clipboard.on('error', (e) => {
          const targetElement = e.trigger;
          const errorBadge = createBadge('Error', 'bg-danger');

          // Append the error badge and remove it after 3 seconds
          targetElement.append(errorBadge);
          removeBadgeAfterDelay(errorBadge, 3000);
        });

        /**
         * Creates a badge element with the given text and CSS class.
         */
        function createBadge(text, cssClass) {
          const badge = document.createElement('span');
          badge.setAttribute('class', `ic-badge badge ${cssClass} float-end`);
          badge.textContent = text;
          return badge;
        }

        /**
         * Removes the given badge element after the specified delay.
         */
        function removeBadgeAfterDelay(badge, delay) {
          setTimeout(() => {
            badge.remove();
          }, delay);
        }
