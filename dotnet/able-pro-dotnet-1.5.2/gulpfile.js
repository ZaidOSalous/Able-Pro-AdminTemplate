...
const path = {
  ...
  "destination": {
    "html": "Ablepro/wwwroot",
    "css": "Ablepro/wwwroot/assets/css",
    "layoutjs": "Ablepro/wwwroot/assets/js",
    "pagesjs": "Ablepro/wwwroot/assets/js/pages",
    "images": "Ablepro/wwwroot/assets/images",
  }
}
...
gulp.task("cleandist", function (callback) {
  del.sync([path.destination.html + "/*/"]);
  callback();
});
// [ common ] task end
...
gulp.task("build-node-modules", function () {
  ...
  npmlodash(required_libs).forEach(function (assets, type) {
    if (type == "css") {
      gulp.src(assets).pipe(gulp.dest("Ablepro/wwwroot/assets/css/plugins"));
    } else {
      gulp.src(assets).pipe(gulp.dest("Ablepro/wwwroot/assets/js/plugins"));
    }
  });
  var required_libs = {
    classic: [
  ...
  npmlodash(required_libs).forEach(function (assets, type) {
    if (type == "classic") {
      gulp
        .src(assets)
        .pipe(gulp.dest("Ablepro/wwwroot/assets/js/plugins/ckeditor/classic"));
    }
    if (type == "inline") {
      gulp
        .src(assets)
        .pipe(gulp.dest("Ablepro/wwwroot/assets/js/plugins/ckeditor/inline"));
    }
    if (type == "balloon") {
      gulp
        .src(assets)
        .pipe(gulp.dest("Ablepro/wwwroot/assets/js/plugins/ckeditor/balloon"));
    }
    if (type == "document") {
      gulp
        .src(assets)
        .pipe(gulp.dest("Ablepro/wwwroot/assets/js/plugins/ckeditor/document"));
    }
  });
  var cpyassets = gulp
    .src(["src/assets/**/*.*", "!src/assets/scss/**/*.*"])
    .pipe(gulp.dest("Ablepro/wwwroot/assets"));
  var cpytinymceassets = gulp
    .src(["node_modules/tinymce/**/*.*"])
    .pipe(gulp.dest("Ablepro/wwwroot/assets/js/plugins/tinymce"));

  var cpytrumbowygassets = gulp
    .src(["node_modules/trumbowyg/dist/**/*.*"])
    .pipe(gulp.dest("Ablepro/wwwroot/assets/js/plugins/trumbowyg"));
...
gulp.task(
  "default",
  gulp.series("cleandist","build-node-modules", "sass", "build-js", compile)
);
//  [ Default task ] end
...
gulp.task(
  "build-prod",
  gulp.series("cleandist","build-node-modules", "sass", "build-js")
);
